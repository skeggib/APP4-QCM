...
public class Logic {
	...
	private String get_stats(ClientConnection prof, String param) {
		...
		stats.setQuestion(running.session.getQuestions().get(numQuestion));
		stats.getQuestion().setId_q(numQuestion);
		stats.setResponses(new HashMap<>());
		for (HashMap<ClientConnection, boolean[]> map : running.responses) {
			for (Entry<ClientConnection, boolean[]> entry : map.entrySet()) {
				...
				stats.getResponses().put(name, entry.getValue());
			}
		}
